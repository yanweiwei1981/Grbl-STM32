...
void serial_write(uint8_t data) {
  ...
  UCSR0B |=  (1 << UDRIE0); 
	...
	USART_ITConfig(USART2,USART_IT_TXE, ENABLE);//2018.09.06
#endif		//end of CPU_MAP_STM32F10X
}
...
void USART2_IRQHandler(void)
{
	...
	uint8_t next_head;
	uint8_t tail; 
	if(USART_GetFlagStatus(USART2 , USART_IT_RXNE)!=RESET)		//接收寄存器非空中断
	{
		data=USART_ReceiveData(USART2);		//读取字符
		// Pick off realtime command characters directly from the serial stream. These characters are
		// not passed into the buffer, but these set system state flag bits for realtime execution.
		//直接从串行流中选取实时命令字符。这些字符不传递到缓冲区，但这些字符设置系统状态标志位用于实时执行。
		switch (data) {
	...
	if (USART_GetITStatus(USART2, USART_IT_TXE)!=RESET) 		//发送寄存器空中断
	{
		...
		{ 
			// Send a byte from the buffer	 发送一个缓冲区的字符
			USART_SendData(USART2, serial_tx_buffer[tail]);						//发送字符
		...
		if (tail == serial_tx_buffer_head) 
		{ 
			USART_ITConfig(USART2, USART_IT_TXE, DISABLE);		//除能发送中断
		}
	}
}